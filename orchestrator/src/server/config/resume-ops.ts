...
export function getResumeGenerationBackend(): ResumeGenerationBackend {
  const value = (
    process.env.RESUME_GENERATION_BACKEND || "native"
  ).toLowerCase();
...
export function getResumeOpsConfig(): {
  baseUrl: string;
  timeoutMs: number;
  theme?: string;
} | null {
  ...
  if (!baseUrl) {
    throw new Error(
      "RESUME_OPS_BASE_URL is required when RESUME_GENERATION_BACKEND=resume_ops",
    );
  }
  ...
  return {
    ...
    timeoutMs: parseInt(
      process.env.RESUME_OPS_REQUEST_TIMEOUT_MS || "120000",
      10,
    ),
